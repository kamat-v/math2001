...
example {a b : ℚ} (h1 : a - 3 = 2 * b) : a ^ 2 - a + 3 = 4 * b ^ 2 + 10 * b + 9 :=
  calc
    a ^ 2 - a + 3 = (a ^ 2 - 6 * a + 9) + 5 * a - 15 + 3 + 15 - 9 := by ring
...
example {z : ℝ} (h1 : z ^ 2 - 2 = 0) : z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1 = 3 :=
  calc
    z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1 = z ^ 4 - z ^ 2 - z ^ 3 + 2 * z + 1 := by ring
    _ = z ^ 2 * (z ^ 2 - 2 + 1) - z * (z ^ 2 - 2) + 1 := by ring
    _ = z ^ 2 *(0 + 1) - z * (0) + 1 := by rw[h1]
    _ = z ^ 2 - 2 + 3 := by ring
    _ = 0 + 3 := by rw[h1]
    _ = 3 := by ring
...
example {t : ℚ} (ht : t ^ 2 - 4 = 0) :
  ...
  calc
    ...
    = t ^ 4 - 8 * t ^ 2 + 16 + 3 * t ^ 3 + 5 * t ^ 2 - 2*t - 18 := by ring
    _ = (t ^ 2 - 4) ^ 2 + 3 * t * (t ^ 2 - 4) + 10 * t + 5 * (t ^ 2 - 4) + 2 := by ring
    _ = (0) ^ 2 + 3 * t * (0) + 10 * t + 5 * (0) + 2 := by rw[ht]
    _ = 10 * t + 2 := by ring


example {x y : ℝ} (h1 : x + 3 = 5) (h2 : 2 * x - y * x = 0) : y = 2 :=
  calc
    y = (y * (5-3) - 2 * (5-3)) / (5-3) + 2 := by ring
    _ = (y * (x + 3 -3) - 2 * (x + 3 - 3)) / (5 - 3) + 2 := by rw[h1]
    _ = (y * x - 2 * x) / 2 + 2 := by ring
